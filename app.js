...
function renderSales(){$('#salesTable').innerHTML=saleGroups().map(g=>{const bal=Number(g.total)-Number(g.paid||0);return `<tr><td>${fmtDate(g.date)}</td><td>${esc(g.client)}</td><td><div class="op-detail">${operationDetail(g.items,'unit_price')}</div></td><td><strong>${money(g.total)}</strong></td><td>${money(g.paid)}</td><td class="money ${bal>0?'negative':''}">${money(bal)}</td><td>${esc(g.method||'')}</td><td><div class="row-actions"><button class="btn secondary small" type="button" onclick="downloadSalePdf('${g.id}')">PDF</button><button class="btn share small" type="button" onclick="shareSalePdf('${g.id}')">Compartir</button><button class="btn danger small" type="button" onclick="deleteSale('${g.id}')">Eliminar</button></div></td></tr>`}).join('')||'<tr><td colspan="8" class="empty">No hay ventas registradas todavía</td></tr>'}
...
function buildSalePdf(group){
  ...
  doc.setFont('helvetica','bold');doc.setFontSize(18);doc.text('Miel & Productos Naturales',left,20);
  doc.setFont('helvetica','normal');doc.setFontSize(10);doc.setTextColor(100);doc.text('Comprobante de venta',left,27);
  doc.setTextColor(35);doc.setFontSize(10);doc.text(`Fecha: ${fmtDate(group.date)}`,left,36);doc.text(`Cliente: ${String(group.client||'Consumidor final').slice(0,80)}`,left,42);
  let y=52;
  doc.setFillColor(246,243,237);doc.rect(left,y-6,right-left,9,'F');doc.setFont('helvetica','bold');doc.setFontSize(9);
