...
function renderEntries(){$('#entriesTable').innerHTML=state.entries.slice().sort(descDate).map(x=>`<tr><td>${fmtDate(x.date)}</td><td>${esc(x.product_name)}</td><td>${x.qty}</td><td>${money(x.unit_cost)}</td><td>${money(x.total)}</td><td>${esc(x.note||'')}</td><td><div class="row-actions"><button class="btn secondary small" type="button" onclick="downloadEntryPdf('${x.id}')">PDF</button><button class="btn share small" type="button" onclick="shareEntryPdf('${x.id}')">Compartir</button><button class="btn danger small" type="button" onclick="deleteEntry('${x.id}')">Eliminar</button></div></td></tr>`).join('')||'<tr><td colspan="7" class="empty">No hay entradas registradas todavía</td></tr>'}
...
function entryPdfName(entry){return `entrada-${String(entry.date||today())}-${safeFileName(entry.product_name)}.pdf`}
function buildEntryPdf(entry){
  if(!window.jspdf?.jsPDF) throw new Error('No se pudo cargar el generador de PDF');
  const {jsPDF}=window.jspdf;
  const doc=new jsPDF({unit:'mm',format:'a4'});
  const left=20, right=190;
  doc.setFont('helvetica','bold'); doc.setFontSize(18); doc.text('Miel & Productos Naturales',left,24);
  doc.setFont('helvetica','normal'); doc.setFontSize(10); doc.setTextColor(100); doc.text('Comprobante de entrada de mercadería',left,31);
  doc.setDrawColor(210); doc.line(left,38,right,38);
  const rows=[
    ['Fecha',fmtDate(entry.date)],
    ['Producto',String(entry.product_name||'')],
    ['Cantidad',String(entry.qty??0)],
    ['Valor unitario',money(entry.unit_cost)],
    ['Total',money(entry.total)]
  ];
  let y=50;
  rows.forEach(([label,value],i)=>{
    doc.setTextColor(100); doc.setFont('helvetica','bold'); doc.setFontSize(10); doc.text(label,left,y);
    doc.setTextColor(30); doc.setFont('helvetica','normal'); doc.setFontSize(i===4?14:11); doc.text(value,70,y);
    if(i<rows.length-1){doc.setDrawColor(235);doc.line(left,y+6,right,y+6)}
    y+=16;
  });
  doc.setDrawColor(210); doc.line(left,y,right,y);
  doc.setTextColor(120); doc.setFontSize(8); doc.text('Generado desde el sistema de stock',left,282);
  return doc;
}
window.downloadEntryPdf=id=>{
  const entry=state.entries.find(x=>x.id===id);
  if(!entry)return toast('No se encontró la entrada');
  try{buildEntryPdf(entry).save(entryPdfName(entry));toast('PDF generado')}catch(err){console.error(err);toast(err.message||'No se pudo generar el PDF')}
};
window.shareEntryPdf=async id=>{
  ...
  try{
    ...
    const file=new File([blob],entryPdfName(entry),{type:'application/pdf'});
    const text=`Entrada de mercadería\nFecha: ${fmtDate(entry.date)}\nProducto: ${entry.product_name}\nCantidad: ${entry.qty}\nValor unitario: ${money(entry.unit_cost)}\nTotal: ${money(entry.total)}`;
    if(navigator.share && (!navigator.canShare || navigator.canShare({files:[file]}))){
      await navigator.share({title:'Comprobante de entrada',text,files:[file]});
      return;
    }
    doc.save(entryPdfName(entry));
    toast('Tu navegador no permite compartir el PDF: se descargó el archivo');
  }catch(err){
    if(err?.name==='AbortError')return;
    console.error(err);toast(err.message||'No se pudo compartir el PDF');
  }
};
