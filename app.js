...
function buildEntryPdf(group){
  ...
  doc.setTextColor(35);doc.setFontSize(10);doc.text(`Fecha: ${fmtDate(group.date)}`,left,36);doc.text(`Proveedor: ${String(group.supplier||'Sin proveedor').slice(0,80)}`,left,42);
  if(group.note)doc.text(`Nota: ${String(group.note).slice(0,100)}`,left,48);
  let y=group.note?58:52;
