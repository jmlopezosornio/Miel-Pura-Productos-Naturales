...
function renderStock(){const q=($('#stockSearch')?.value||'').toLowerCase();const rows=state.products.filter(p=>p.name.toLowerCase().includes(q));$('#stockTable').innerHTML=rows.map(p=>`<tr><td><strong>${esc(p.name)}</strong></td><td>${esc(p.presentation||'')}</td><td><span class="badge ${p.stock<=2?'low':p.stock<=5?'':'ok'}">${p.stock}</span></td><td>${money(p.cost)}</td><td>${money(p.price)}</td><td>${money(p.stock*p.cost)}</td><td>${money(p.stock*p.price)}</td><td><button class="danger-link" onclick="editProduct('${p.id}')">Editar</button></td></tr>`).join('')||'<tr><td colspan="8" class="empty">Sin productos</td></tr>'}
function renderEntries(){$('#entriesTable').innerHTML=state.entries.slice().sort(descDate).map(x=>`<tr><td>${fmtDate(x.date)}</td><td>${esc(x.product_name)}</td><td>${x.qty}</td><td>${money(x.unit_cost)}</td><td>${money(x.total)}</td><td>${esc(x.note||'')}</td></tr>`).join('')||'<tr><td colspan="6" class="empty">No hay entradas registradas todavía</td></tr>'}
function renderSales(){$('#salesTable').innerHTML=state.sales.slice().sort(descDate).map(x=>{const bal=Number(x.total)-Number(x.paid||0);return `<tr><td>${fmtDate(x.date)}</td><td>${esc(x.client||'Consumidor final')}</td><td>${esc(x.product_name)}</td><td>${x.qty}</td><td>${money(x.total)}</td><td>${money(x.paid)}</td><td class="money ${bal>0?'negative':''}">${money(bal)}</td><td>${esc(x.method||'')}</td></tr>`}).join('')||'<tr><td colspan="8" class="empty">No hay ventas registradas todavía</td></tr>'}
...
(async()=>{await initStorage();render()})();
